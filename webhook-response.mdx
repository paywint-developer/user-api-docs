...
Sent when a payment is initiated.
...
Sent when a payment link can no longer be used because it has passed its expiry or has been closed. No further payment attempts can be initiated against this link; users must request a new link (which will create a new payment).
